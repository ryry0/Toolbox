...
float nm_fdFirstDer5(const ring_buffer_t r_buff, float dt) {
  ...
  const float B = 5.0f/4.0f;
  const float C = -10.0f/3.0f;
  const float D = 5.0f;
  const float E = -5.0f;
  const float F = 137.0f/60.0f;
...
float nm_fdSecondDer5(const ring_buffer_t r_buff, float dt) {
  ...
  const float B = -61.0f/12.0f;
  const float C = 13.0f;
  const float D = -107.0f/6.0f;
  const float E = 77.0f/6.0f;
  const float F = -15.0f/4.0f;
...
float nm_dIntegrate4(const ring_buffer_t prev_vals, float du, float dt) {
  const float A = 18.0f/11.0f;
  const float B = -9.0f/11.0f;
  const float C = 2.0f/11.0f;
  const float D = 6.0f/11.0f;
...
size_t nm_genRange(float start, float increment, float end, float **array) {
  const size_t arr_size = (size_t) ceil(fabs((end - start)/increment));
  float *range = malloc(arr_size*sizeof(float));

  for (size_t i = 0; i < arr_size; ++i) {
    range[i] = start + ((float)i)*increment;
  }
